...
def iou_xyxy_torch(boxes1, boxes2):
    ...
    inter_section = torch.max(
        right_down - left_up, torch.zeros_like(right_down)
    )
    inter_area = inter_section[..., 0] * inter_section[..., 1]
    union_area = boxes1_area + boxes2_area - inter_area
    IOU = 1.0 * inter_area / union_area
...
def GIOU_xywh_torch(boxes1, boxes2):
    ...
    enclose_section = torch.max(
        enclose_right_down - enclose_left_up,
...
def CIOU_xywh_torch(boxes1, boxes2):
    ...
    outer_diagonal_line = torch.pow(outer[..., 0], 2) + torch.pow(
        outer[..., 1], 2
    )
    ...
    v = (4 / (math.pi ** 2)) * torch.pow(
        torch.atan(
            (boxes1_size[..., 0] / torch.clamp(boxes1_size[..., 1], min=1e-6))
        )
        - torch.atan(
