...
if not args.no_animation:
    try:
        ...
        print(
            '"opencv-python" not found, please install to visualize the results.'
        )
        args.no_animation = True
...
if show_animation:
    os.makedirs(
        os.path.join(results_files_path, "images", "detections_one_by_one")
    )
    ...
    for txt_file in dr_files_list:
        ...
        for line in lines:
            try:
                ...
                error_msg = (
                    "Error: File " + txt_file + " in the wrong format.\n"
                )
...
with open(results_files_path + "/results.txt", "w") as results_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            if show_animation:
                status = (
                    "NO MATCH FOUND!"  # status is only used in the animation
                )
            # set minimum overlap
            ...
            if show_animation:
                ...
                if (
                    ...
                    bbgt = [
                        int(round(float(x))) for x in gt_match["bbox"].split()
                    ]
                    cv2.rectangle(
                        img,
                        (bbgt[0], bbgt[1]),
        ...
        sum_AP += ap
        text = class_name + " AP = {0:.2f}%".format(
            ap * 100
